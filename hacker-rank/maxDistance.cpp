...
int maxPathSumUtil(struct Node *root, int &res)
{
    ...
    cout << "L: " << root->data << " " << res << endl;
    int rLPSum = maxPathSumUtil(root->right, res);
    cout << "R: " << root->data << " " << res << endl;
 
    // Find the maximum path sum passing through root
    res = max(max((lLPSum+rLPSum+root->data), max(lLPSum, rLPSum)), res);
    cout << "F: " << root->data << " " << lLPSum+rLPSum+root->data << " " << lLPSum << " " << rLPSum << " " << res << endl;
